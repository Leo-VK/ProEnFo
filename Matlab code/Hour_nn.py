...
torch.manual_seed(1)
breakpoint_new=loadmat('E:/ETH learning material/Master Thesis/Code/Allplots/Allplots_new/20200906/breakpoint_new.mat')
...
store_slope=loadmat('E:/ETH learning material/Master Thesis/Code/Allplots/Allplots_new/20200906/store_slope.mat')['store_slope']
...
# N, D_in, H1, H2,D_out = 64, 1019, 1024, 2048,1
...
epochs =350
learning_rate = 1e-2
# optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, betas=(0.9, 0.999),weight_decay=1e-8)#
...
decayRate = 0.5
# decay_schedule= [200,250]
# my_lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,milestones=decay_schedule, gamma=decayRate)
my_lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=decayRate)
j=0
...
# for j in range(epochs):
    ...
    error_dist=np.divide(Y_predict-Test_y,Test_y)
    ...
    my_lr_scheduler.step(FEPC_average_id)
    if  FEPC_average_id<22.5  and MAPE_AVE>5.89 and MAPE_AVE<6 and OFR>52 and OFR<54:
        break
