...
class TFTTimeFeatureEmbedding(nn.Module):
    def __init__(self, configs,d_model, embed_type='timeF', freq='h'):
        super(TFTTimeFeatureEmbedding, self).__init__()
        d_inp = configs.ex_dim
...
class TFTEmbedding(nn.Module):
    def __init__(self, configs):
        ...
        self.static_pos = []
        self.observed_pos = list(range(configs.c_in))
        ...
        self.known_embedding = TFTTemporalEmbedding(configs.d_model, configs.embed, configs.freq) \
            if configs.embed != 'timeF' else TFTTimeFeatureEmbedding(configs,configs.d_model, configs.embed, configs.freq)
...
class TemporalFusionDecoder(nn.Module):
    def __init__(self, configs):
        ...
        self.out_projection = nn.Linear(configs.d_model, configs.c_in)
...
class TFT(nn.Module):
    def __init__(self, configs):
        ...
        # Number of variables
        self.static_len = 0
        self.observed_len = configs.c_in
        self.known_len = configs.ex_dim
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        return self.proj(dec_out.unsqueeze(-1))  # [B, T, D]