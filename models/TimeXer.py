...
class TimeXer(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        en_embed, n_vars = self.en_embedding(x_enc[:, :, -self.ex_dim:].permute(0, 2, 1))
        ex_embed = self.ex_embedding(x_enc[:, :, :-self.ex_dim], x_mark_enc)
        ...
        if self.use_norm:
            ...
            dec_out = dec_out * (stdev[:, 0, -self.ex_dim:].unsqueeze(1).repeat(1, self.pred_len, 1))
            dec_out = dec_out + (means[:, 0, -self.ex_dim:].unsqueeze(1).repeat(1, self.pred_len, 1))
