...
class ex_Mixer(nn.Module):
    def __init__(self, configs):
        super(ex_Mixer, self).__init__()
        ...
        self.initialize_to_zeros(self.mixer_layers)
        self.initialize_to_zeros(self.temporal_projection)
    ...
    def forward(self, X,X_ex):
        XT = torch.cat([X.mean(dim=-1),X_ex],-1)
        XT = self.mixer_layers(XT)
        x_temp = feature_to_time(XT)
        x_temp = self.temporal_projection(x_temp)
        XT = time_to_feature(x_temp)
        XT = self.proj(XT.unsqueeze(-1))
        return X+XT