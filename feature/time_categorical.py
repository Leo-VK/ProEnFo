...
def add_datetime_features_with_lag(data: pd.DataFrame, feature_strategies: List[TimeCategorical], target_lag: List[int], target_preds: List[int], freq: str = 'H') -> pd.DataFrame:
...
def add_datetime_features_with_lag_pred(data: pd.DataFrame, feature_strategies: List[TimeCategorical], target_lag: List[int], target_preds: List[int], freq: str = 'H') -> pd.DataFrame:
    """Add datetime features with and without target lags and forward preds"""
    original_index = data.index.to_series()
    ...
    for strategy in feature_strategies:
        ...
        column_name = f"{strategy.name}"
        new_columns.append(pd.DataFrame(feature, columns=[column_name], index=original_index))

    # Add lagged features
    for lag in reversed(target_lag):
        lagged_index = original_index.shift(lag)  # Shift the index backward by 'lag' units
        ...
        for strategy in feature_strategies:
            ...
            feature.index = original_index  # Set the feature index back to the original index
            column_name = f"{strategy.name}_t-{lag}"
            new_columns.append(pd.DataFrame(feature, columns=[column_name], index=original_index))

    # Add forward pred features
    for pred in target_preds:
        forward_index = original_index.shift(-pred)  # Shift the index forward by 'pred' units
        
        for strategy in feature_strategies:
            feature = strategy.calculate_feature(forward_index)  # Calculate the feature using the forward index
            if feature.sum() == 0:
                raise ValueError("Resolution does not support this TimeCategorical")
            if strategy.transformer is not None:
                feature = strategy.transformer.fit_transform(feature)
            ...
            feature.index = original_index  # Set the feature index back to the original index
            column_name = f"{strategy.name}_t+{pred}"
            new_columns.append(pd.DataFrame(feature, columns=[column_name], index=original_index))

    # Concatenate new_columns to the original data
