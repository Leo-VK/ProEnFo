...
from feature.time_lag import lag_target, pred_target,rename_lag_series
...
class LagStrategy(FeatureExternalSelectionStrategy):
    ...
    def __init__(self, lag_strategy_by_name: Dict[str, FeatureLagSelectionStrategy],
                 pred_strategy_by_name: Dict[str, FeatureLagSelectionStrategy]):
        super().__init__(list(lag_strategy_by_name.keys()))
        self.lag_strategy_by_name = lag_strategy_by_name
        self.pred_strategy_by_name = pred_strategy_by_name
    ...
    def select_features(self, data: pd.DataFrame, horizon: int) -> Tuple[
        ...
        for name, strategy in self.pred_strategy_by_name.items():
            preds[name] = strategy.select_features(data[name], horizon)
            features = pred_target(features, name, preds[name])

        return features, lags, preds
