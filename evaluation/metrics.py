...
class ErrorMetric(ABC):
    ...
    def calculate_mean_error(self,
        ...
        output = self.calculate_instant_error(y_true, y_pred)
        output = output.multiply(weights, axis=0).div(weights.sum()).sum()
        return output
...
class CRPSError(ErrorMetric):
    """Continuous Ranked Probability Score (CRPS). This is a proper scoring rule"""
    ...
    def __init__(self,
        ...
        self.quantiles = check_quantile_list(quantiles)
        self.weights = weights if weights is not None else uniform_quantile_weighting(quantiles)
    def F(self,y, quantiles, probabilities):
        if y < quantiles[0]:
            return 0
        elif y >= quantiles[-1]:
            return 1
            ...
            for i in range(len(quantiles) - 1):
                if quantiles[i] <= y < quantiles[i + 1]:
                    return probabilities[i] + (y - quantiles[i]) / (quantiles[i + 1] - quantiles[i]) * (probabilities[i + 1] - probabilities[i])

    def integrand(self,y, quantiles, probabilities, x):
        return (self.F(y, quantiles, probabilities) - (1 if y >= x else 0)) ** 2

    def CRPS(self,quantiles, probabilities, x):
        integral, _ = quad(self.integrand, -np.inf, np.inf, args=(quantiles, probabilities, x))
        return integral

    def calculate_instant_error(self, y_true: pd.Series, y_pred: pd.DataFrame) -> pd.Series:
        # Generate cumulative distribution functions (CDFs) from quantile predictions

        # Calculate CRPS for each instance
        crps_errors = [self.CRPS(self.quantiles,y_pred.iloc[i].tolist(),[y_true.iloc[i]]) for i in range(len(y_true))]


        return pd.Series(crps_errors, index=y_true.index)
